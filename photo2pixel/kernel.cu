...
void cpu_function() {
    // --- 参数配置 ---
    string imgPath = "input3.jpg";
    int pixel_size = 4;   // 马赛克大小
    int kernel_size = 5;  // 平滑范围
    ...
    int w = img_in.cols;
    int h = img_in.rows;
    cout << "CPU Processing - Image: " << w << "x" << h << endl;

    // 2. 创建临时和输出图像
    Mat img_temp = Mat::zeros(h, w, CV_8UC3);
    Mat img_out = Mat::zeros(h, w, CV_8UC3);

    cout << "Start CPU Processing..." << endl;

    // --- Step 1: 预处理 (亮度/对比度 + 桑原滤波) ---
    for (int y = 0; y < h; ++y) {
        for (int x = 0; x < w; ++x) {
            // 定义桑原滤波的 4 个区域偏移
            int r = kuwahara_size / 2;
            int regions[4][4] = {
                {-r, -r, 0, 0}, {0, -r, r, 0}, {-r, 0, 0, r}, {0, 0, r, r}
            };
            ...
            for (int k = 0; k < 4; ++k) {
                float mean[3] = { 0, 0, 0 };
                float sq_mean[3] = { 0, 0, 0 };
                int count = 0;

                for (int dy = regions[k][1]; dy <= regions[k][3]; ++dy) {
                    for (int dx = regions[k][0]; dx <= regions[k][2]; ++dx) {
                        // 边界处理
                        int px = max(0, min(x + dx, w - 1));
                        int py = max(0, min(y + dy, h - 1));

                        // 读取像素 (BGR格式)
                        Vec3b p = img_in.at<Vec3b>(py, px);

                        // --- 亮度与对比度增强 ---
                        float c[3];
                        for (int i = 0; i < 3; ++i) {
                            c[i] = max(0.0f, min(1.0f, ((p[i] / 255.0f * brightness) - 0.5f) * contrast + 0.5f));
                            mean[i] += c[i];
                            sq_mean[i] += c[i] * c[i];
                        }
                        count++;
                ...
                float total_var = 0;
                for (int i = 0; i < 3; ++i) {
                    total_var += (sq_mean[i] - mean[i] * mean[i]);
                }
                ...
                if (total_var < min_variance) {
                    min_variance = total_var;
                    for (int i = 0; i < 3; ++i) {
                        final_color[i] = mean[i];
                    }
                }
            }

            // 输出预处理后的像素
            img_temp.at<Vec3b>(y, x) = Vec3b(
                (uchar)(final_color[0] * 255),
                (uchar)(final_color[1] * 255),
                (uchar)(final_color[2] * 255)
            );
        }
    }
    ...
    // --- Step 2: 像素化 (分层统计) ---
    int num_blocks_x = (w + pixel_size - 1) / pixel_size;
    int num_blocks_y = (h + pixel_size - 1) / pixel_size;
        ...
        for (int bx = 0; bx < num_blocks_x; ++bx) {
            ...
            for (int dy = -r; dy <= r; ++dy) {
                for (int dx = -r; dx <= r; ++dx) {
                    ...
                    Vec3b p = img_temp.at<Vec3b>(py, px);
            ...
            // 计算该层的平均色
            Vec3b final_color(0, 0, 0);
            if (max_count > 0) {
                final_color[0] = bin_sums[best_bin][0] / max_count;
                final_color[1] = bin_sums[best_bin][1] / max_count;
                final_color[2] = bin_sums[best_bin][2] / max_count;
            }
            ...
            for (int dy = 0; dy < pixel_size; ++dy) {
                for (int dx = 0; dx < pixel_size; ++dx) {
                    ...
                    int py = start_y + dy;
                    if (px < w && py < h) {
                        img_out.at<Vec3b>(py, px) = final_color;
                    }
                }
            }
        }
    ...
    cout << "Pixelization done." << endl;

    // 3. 保存结果
    imwrite("result_cpu.png", img_out);
    cout << "CPU Result saved to result_cpu.png" << endl;
}
...
int main() {
    ...
    cout << "CPU Total Time: " << cpu_duration.count() << " ms" << endl;
    ...
    cudaMalloc((void**)&d_temp, img_size);
    ...
    chrono::duration<double, milli> gpu_duration = gpu_end - gpu_start;
    cout << "GPU Total Time (including memory transfer): " << gpu_duration.count() << " ms" << endl;
    ...
    cout << "Speedup:     " << (cpu_duration.count() / gpu_duration.count()) << "x" << endl;
    cout << "======================================" << endl;
